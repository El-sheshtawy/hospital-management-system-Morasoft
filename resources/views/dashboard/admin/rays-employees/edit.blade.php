<!-- Modal -->
<div class="modal fade" id="edit{{ $rays_employee->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
     aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.rays-employees.update', $rays_employee) }}" method="post">
                @method('PUT')
                @csrf
                @csrf
                <div class="modal-body">
                    <label for="exampleInputPassword1">الاسم</label>
                    <input type="text" value="{{$rays_employee->name}}" name="name" class="form-control"><br>

                    <label for="exampleInputPassword1">البريد الالكتروني</label>
                    <input type="email" value="{{$rays_employee->email}}" name="email" class="form-control"><br>
