			...
			<div class="dropdown user-pro-body">
				...
				<div class="user-info">
                    ...
                    @if(auth()->user()->hasRole('super_admin'))
                            <span class="mb-0 text-muted">سيد الناس عم الناس</span>
                    @elseif(auth()->user()->hasRole('admin'))
					    <span class="mb-0 text-muted">System Admin Role</span>
                    @elseif(auth()->user()->hasRole('doctor'))
                        <span class="mb-0 text-muted">Doctor Role</span>
                    @elseif(auth()->user()->hasRole('rays_employee'))
                        <span class="mb-0 text-muted">Rays Employee Role</span>
                    @else
                        NO Permissions To access that page !!
                    @endif
				</div>
			</div>
		</div>
		<ul class="side-menu">
			<li style="margin-bottom: 15px">{{trans('dashboard/main-sidebar_trans.Main')}}</li>
			<li class="slide">
...
@can('show-sections')
            <li class="side-item side-item-category">Main Sections</li>
...
@if(auth()->user()->can('show-single-services') || auth()->user()->can('show-groups-services') ||
auth()->user()->can('show-insurances') || auth()->user()->can('show-ambulances'))
            <li class="side-item side-item-category">Multi Services</li>
			<li class="slide">
				...
				<ul class="slide-menu">
            @endif
@can('show-single-services')
					<li>
						<a class="slide-item" href="{{route('admin.single-service.index')}}">
							{{trans('dashboard/main-sidebar_trans.Single_service')}}
						</a>
					</li>
@endcan

@can('show-groups-services')
					<li>
						<a class="slide-item" href="{{ route('admin.group-services.index') }}">
...
@can('show-insurances')
					<li>
						<a class="slide-item" href="{{ route('admin.insurances.index') }}">
...
@can('show-ambulances')
					<li>
						<a class="slide-item" href="{{route('admin.ambulances.index')}}">
...
@can('show-patients')
			<li class="slide">
				...
				<ul class="slide-menu">
					<li><a class="slide-item" href="{{ route('admin.patients.index') }}">جميع المرضي</a></li>
				</ul>
			</li>
@endcan

@if(auth()->user()->can('show-invoices') || auth()->user()->can('show-doctor-related-invoices')
|| auth()->user()->can('show-rays-employee-related-invoices'))
			...
			<li class="slide">
				...
				<ul class="slide-menu">
					<li><a class="slide-item" href="{{ route('admin.single-invoice.index') }}">فاتورة خدمة مفردة</a></li>
					<li><a class="slide-item" href="{{ route('admin.group-invoices.index') }}">فاتورة خدمة مجمعة</a></li>
				</ul>
			</li>
@endif

        @if(auth()->user()->can('show-receipts') || auth()->user()->can('show-payments'))
			<li class="side-item side-item-category">قسم الحسابات</li>
			<li class="slide">
				<a class="side-menu__item" data-toggle="slide" href=""><svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon" viewBox="0 0 24 24" ><path d="M0 0h24v24H0V0z" fill="none"/><path d="M15 11V4H4v8.17l.59-.58.58-.59H6z" opacity=".3"/><path d="M21 6h-2v9H6v2c0 .55.45 1 1 1h11l4 4V7c0-.55-.45-1-1-1zm-5 7c.55 0 1-.45 1-1V3c0-.55-.45-1-1-1H3c-.55 0-1 .45-1 1v14l4-4h10zM4.59 11.59l-.59.58V4h11v7H5.17l-.58.59z"/></svg><span class="side-menu__label">Money</span><i class="angle fe fe-chevron-down"></i></a>
				<ul class="slide-menu">
					<li><a class="slide-item" href="{{ route('admin.receipts.index') }}">سند قبض</a></li>
					<li><a class="slide-item" href="{{ route('admin.payments.index') }}">سند صرف</a></li>
				</ul>
			</li>
@endif
			...
			<li class="slide">
				<a class="side-menu__item" data-toggle="slide" href=""><svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon" viewBox="0 0 24 24"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M13 4H6v16h12V9h-5V4zm3 14H8v-2h8v2zm0-6v2H8v-2h8z" opacity=".3"/><path d="M8 16h8v2H8zm0-4h8v2H8zm6-10H6c-1.1 0-2 .9-2 2v16c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm4 18H6V4h7v5h5v11z"/></svg><span class="side-menu__label">قائمة الاشعة</span><i class="angle fe fe-chevron-down"></i></a>
				<ul class="slide-menu">
@can('show-rays-employees')
                    <li><a class="slide-item" href="{{ route('admin.rays-employees.index') }}">موظفي لاشعة</a></li>
@endcan

    <a class="side-menu__item" data-toggle="slide" href=""><svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon" viewBox="0 0 24 24" ><path d="M0 0h24v24H0z" fill="none"/><path d="M12 4c-4.41 0-8 3.59-8 8s3.59 8 8 8c.28 0 .5-.22.5-.5 0-.16-.08-.28-.14-.35-.41-.46-.63-1.05-.63-1.65 0-1.38 1.12-2.5 2.5-2.5H16c2.21 0 4-1.79 4-4 0-3.86-3.59-7-8-7zm-5.5 9c-.83 0-1.5-.67-1.5-1.5S5.67 10 6.5 10s1.5.67 1.5 1.5S7.33 13 6.5 13zm3-4C8.67 9 8 8.33 8 7.5S8.67 6 9.5 6s1.5.67 1.5 1.5S10.33 9 9.5 9zm5 0c-.83 0-1.5-.67-1.5-1.5S13.67 6 14.5 6s1.5.67 1.5 1.5S15.33 9 14.5 9zm4.5 2.5c0 .83-.67 1.5-1.5 1.5s-1.5-.67-1.5-1.5.67-1.5 1.5-1.5 1.5.67 1.5 1.5z" opacity=".3"/><path d="M12 2C6.49 2 2 6.49 2 12s4.49 10 10 10c1.38 0 2.5-1.12 2.5-2.5 0-.61-.23-1.21-.64-1.67-.08-.09-.13-.21-.13-.33 0-.28.22-.5.5-.5H16c3.31 0 6-2.69 6-6 0-4.96-4.49-9-10-9zm4 13h-1.77c-1.38 0-2.5 1.12-2.5 2.5 0 .61.22 1.19.63 1.65.06.07.14.19.14.35 0 .28-.22.5-.5.5-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.14 8 7c0 2.21-1.79 4-4 4z"/><circle cx="6.5" cy="11.5" r="1.5"/><circle cx="9.5" cy="7.5" r="1.5"/><circle cx="14.5" cy="7.5" r="1.5"/><circle cx="17.5" cy="11.5" r="1.5"/></svg><span class="side-menu__label">قائمة الاشعة</span><i class="angle fe fe-chevron-down"></i></a>
    <ul class="slide-menu">
@if(auth()->user()->hasRole('admin'))
            <li><a class="slide-item" href="{{ route('admin.rays-employees.index') }}">جميع الاشعة المكتملة</a></li>
            <li><a class="slide-item" href="{{ route('admin.rays-employees.index') }}">جميع الاشعة قيد التنفيذ</a></li>
@elseif(auth()->user()->hasRole('doctor'))
            <li><a class="slide-item" href="{{ route('rays.completed') }}">جميع الاشعة المكتملة</a></li>
            <li><a class="slide-item" href="{{ route('rays.pending') }}">جميع الاشعة قيد التنفيذ</a></li>
@elseif(auth()->user()->hasRole('rays_employee'))
            <li><a class="slide-item" href="{{ route('rays.completed') }}">جميع الاشعة المكتملة</a></li>
            <li><a class="slide-item" href="{{ route('rays.pending') }}">جميع الاشعة قيد التنفيذ</a></li>
@else
...
@endif
			...
			<li class="slide">
				<a class="side-menu__item" href=""><svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon"  viewBox="0 0 24 24" ><path d="M0 0h24v24H0V0z" fill="none"/><path d="M5 5h4v4H5zm10 10h4v4h-4zM5 15h4v4H5zM16.66 4.52l-2.83 2.82 2.83 2.83 2.83-2.83z" opacity=".3"/><path d="M16.66 1.69L11 7.34 16.66 13l5.66-5.66-5.66-5.65zm-2.83 5.65l2.83-2.83 2.83 2.83-2.83 2.83-2.83-2.83zM3 3v8h8V3H3zm6 6H5V5h4v4zM3 21h8v-8H3v8zm2-6h4v4H5v-4zm8-2v8h8v-8h-8zm6 6h-4v-4h4v4z"/></svg><span class="side-menu__label">Widgets</span><span class="badge badge-warning side-badge">Hot</span></a>
			</li>
			<li class="slide">
